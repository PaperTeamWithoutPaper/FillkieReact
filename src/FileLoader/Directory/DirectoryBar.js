    ...
    return(
        ...
        <div 
            ...
            onClick={()=>{setIsclicked(isclicked*(-1))}}>
                <img className={isclicked==-1?"DirectoryPath-folder-arrow-normal":"DirectoryPath-folder-arrow-clicked"} src={require('./icon/arrow.png')}></img>
                {<div>📁</div>} 
                <div style={{marginLeft:'5px'}}>{props.title}</div>
            </div>
            {files.map((e)=>{if(e.type==3 && isclicked==1){return(<DirectoryBar title={e.title}></DirectoryBar>)}else if(isclicked==1){return(<FileBar depth={props.depth+1} title={e.title}></FileBar>)}})}
        </div>
